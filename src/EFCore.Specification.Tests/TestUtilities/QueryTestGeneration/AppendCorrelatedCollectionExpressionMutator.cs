...
namespace Microsoft.EntityFrameworkCore.TestUtilities.QueryTestGeneration
{
    public class AppendCorrelatedCollectionExpressionMutator : ExpressionMutator
    {
        ...
        public override Expression Apply(Expression expression, Random random)
        {
            ...
            var enumerableWhere = typeof(Enumerable).GetMethods().Where(m => m.Name == nameof(Enumerable.Where) && m.GetParameters()[1].ParameterType.GetGenericArguments().Count() == 2).Single();
            var where = enumerableWhere.MakeGenericMethod(collectionElementType);

            var enumerableToList = typeof(Enumerable).GetMethods().Where(m => m.Name == nameof(Enumerable.ToList)).Single();
            var toList = enumerableToList.MakeGenericMethod(collectionElementType);
